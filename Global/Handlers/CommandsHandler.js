...
const MessageCommandsHandler = async function (client, message) {
    ...
    if (cmd) {
        ...
        if (cmd.Category === 'Root' && !client.system.ownerID.includes(message.author.id)) return;
...
const SlashCommandsHandler = async function (client, interaction) {
    ...
    if (cmd.Category === 'Root' && !client.system.ownerID.includes(interaction.user.id)) return;
