...
const { FileManager } = require("../Handlers/index.js"),
  { Logger, Validator } = require("../Helpers/index.js");
...
class ertuClient extends Client {
  constructor(Settings) {
    ...
    (this.ertu = Settings), (this.commands = new Collection());
    ...
    this.giveawaysManager = new GiveawaysManager(this, {
      storage: resolve(__dirname, "../Settings/Server/Giveaways.json"),
      default: {
        botsCanWin: true,
        embedColor: "#0a0000",
        buttonEmoji: "🎉",
  ...
  async spawn() {
      ...
      try {
        ...
        if (event && event.System) {
          if (typeof event.execute === "function") {
            ...
            this.logger.error(
              `Event file (${file}) is missing the 'execute' function.`
            );
            error += 1;
          }
      ...
      try {
        ...
        if (valid) {
          ...
          if (data.Command.Slash) {
            if (this.slashCommands.has(data.Name))
              throw new Error(
                `Slash Command '${data.Name}' already registered`
              );
    ...
    this.logger.success(
      `Loaded ${
        this.commands.size + this.slashCommands.size
      } commands (Prefix: ${this.commands.size} Slash: ${
        this.slashCommands.size
      }`
    );
    this.logger.success(
      `Loaded ${success + error} events. Success (${success}) Failed (${error})`
    );

    if (this.slashCommands.size > 100)
      this.logger.error("A maximum of 100 slash commands can be enabled");

    return super.login(this.ertu.Token).then((app) => {
      this.on("ready", async (client) => {
        loadFont(
          "Kanit",
          resolve(__dirname, "../Assets/Fonts", "Kanit-Regular.ttf")
        );

        for (const guild of [...client.guilds.cache.values()]) {
          Promise.all([guild.fetchSettings()]);
        }
        ...
        client.giveawaysManager.on(
          ...
          (giveaway, member, interaction) => {
            return interaction.reply({
              content: `🎉 Tebrikler ${member}, çekilişe katıldınız`,
              ephemeral: true,
            });
          }
        );
        ...
        client.giveawaysManager.on(
          ...
          (giveaway, member, interaction) => {
            return interaction.reply({
              content: `🎉 ${member}, çekilişten başarıyla çıktınız`,
              ephemeral: true,
            });
          }
        );
