...
module.exports = {
    ...
    messageRun: async (client, message, args, ertu, embed) => {
        ...
        if (!client.staff.check(member, ertu)) {
            ...
            const question = await message.channel.send({
                components: message.member.permissions.has(PermissionFlagsBits.Administrator) ? [subRow, middleRow, topRow]
                    : ertu.settings.founders.some(r => message.member.roles.cache.has(r)) ? [subRow, middleRow, topRow]
                        : message.member.roles.cache.find(r => r.name.includes('Yetkili Alım Sorumlusu')) ? [subRow]
                        : message.member.roles.cache.find(r => r.name.includes('Yetkili Alım Denetleyici')) ? [subRow, middleRow]
                            : message.member.roles.cache.find(r => r.name.includes('Yetkili Alım Lideri')) ? [subRow, middleRow, topRow]
                                : [],
                embeds: mappedData.length ? [
                    new EmbedBuilder({
                        color: client.getColor('random'),
